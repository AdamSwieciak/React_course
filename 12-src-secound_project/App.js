...
import "./App.css";
import PoolVal from "./ValidationComponent/ValidationComponent";
...
class App extends Component {
  ...
  removecharChandler = index => {
    const text = this.state.userinput.split("");
    text.splice(index, 1);
    // make a array[a,b,c,f]
  ...
  render() {
    const charList = this.state.userinput.split("").map((ch, index) => {
      return (
        <Char
          character={ch}
          key={index}
          clicked={() => this.removecharChandler(index)}
        />
      );
    });
    ...
    return (
      <div className="App">
        ...
        <PoolVal value={this.state.userinput.length} />
        {charList}
      </div>
    );
  }
